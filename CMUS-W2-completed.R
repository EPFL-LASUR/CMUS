...
palem_mca <- palem %>%
  mutate(                                                        # "mutate" either transform existing / create new variables based on the name.
    ...
    alim_dri_cof =                                                # How many coffees individuals drink each week ?
      case_when(ali_q1_1 < 1 ~ 1, ali_q1_1 < 6 ~ 2,
                ali_q1_1 < 11 ~ 3, ali_q1_1 < 21 ~ 4,
                is.na(ali_q1_1) ~ NA, T ~ 5),
    alim_dri_cof = labelled(
      alim_dri_cof, 
      labels = c("none" = 1, "1 to 5" = 2, "6 to 10" = 3, 
                 "11 to 20" = 4, "21 or more" = 5), 
...
as_tibble(mca$var$v.contrib, rownames = "variables") %>%          # Dimension 2 is related to transport modes, home appliances and meat consumption.
  arrange(-dim.2) %>%
  filter(dim.2 > 100 / (nrow(mca$var$v.contrib)))
...
as_tibble(mca$var$coord, rownames = "categories") %>%             # We get the "coordinates" from the "variables" (categories) from the mca object. We also specify that the table we expect should include the row names as a column named "categories".
    ...
    as_tibble(mca$var$coord, rownames = "categories") %>%
      ...
      bind_cols(as_tibble(mca$var$contrib) %>%
                  rename(dim1_contrib = `dim.1`, 
                         ...
                         dim4_contrib = `dim.4`, 
                         dim5_contrib = `dim.5`)) %>%
      mutate(sig = dim2_contrib > 100 / nrow(mca$var$contrib),    # Note here we are doing the same for DIMENSION 2.
                     facet = "dim2")) %>% 
  ggplot(                                                         # "ggplot" is the package for plotting the results - it always start like this, but from now on we will use "+" and not "%>%" anymore because we will be adding layers rather than "piping" operations.
    aes(x = dim1_coord, y = dim2_coord)) +                        # "aes()" lets us specify what are the variables to be plotted - here we want a scatter plot of each category with both their coordinates on dimension 1 and 2.
  ...
  facet_grid(. ~ facet,                                           # "facet_grid" lets ggplot know that we are not plotting one but two graphs: one where we care about categories' significance for dimension 1, and the other for dimension 2.
             labeller = as_labeller(                              # "labeller" lets us add a legend to both facets - here we chose the name of each dimension based on our interpretation.
               c("dim1" = "Significant categories for the 'Standards' axis", 
                 "dim2" = "Significant categories for the 'Consumption' axis")
               )) +
  ...
  labs(                                                           # "labs" lets us choose the labels for both our axes.
    ...
    y = paste0("+ Unsustainable // Sustainable + \n (", round(
      mca$eig$mrate[2], 1), "% var. ex.)"))

# What follows is a copy/paste but for the plane made of
# ... dimension 1 and 3.
...
ggcloud_indiv(mca, axes = c(1, 2)) %>%                            # For dimension 3, let's verify if it's indeed related to the (non-)urban context... 
