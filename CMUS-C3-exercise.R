...
### ASSESSMENT 1 ----
as_tibble(..., rownames = "ballot") %>%                             # 1 - Retrieve the coordinates of each ballot.
  slice(...) %>%                                                    # 2 - Keep only the 2024 highway ballot (row n° is 376).
  mutate(...) %>%                                                   # 3 - Create a new variable named "ballot" where each row has the same string value "coord". 
  bind_rows(as_tibble(                                              # 4 - Bind one new row which is exactly the same but for the contributions rather than the coordinates. Each row should have the same string value "contrib" for the "ballot" variable.
    ..., rownames = "ballot") %>% 
      slice(...) %>% 
      mutate(...)) %>%
  pivot_longer(...) %>%                                             # 5 - Create a longer format with three columns : ballot (with values "coord" or "contrib"), dim (for each dimension) and value (for the coordinates/contributions). You should get a 20x3 tibble.
  pivot_wider(...)                                                  # 6 - Create a wider format with three columns : dim (for each dimension), coord (for the coordinates) and contrib (for the contribution). You should get a 10x3 tibble.
...
### ASSESSMENT 2 ----                                               # Here you can change which dimensions are represented on the graph and analyze the position of the 2024 ballot for each resulting plane. Don't forget to change the name of the axes!
...
### ASSESSMENT 3 ----
as_tibble(...) %>%                                                  # 1 - Retrieve the coordinates of each municipality from the PCA.
  bind_cols(...) %>%                                                # 2 - Bind the "entity" column from the "data" object you have in your environment.
  left_join(...) %>%                                                # 3 - Join all the information from the "municipalities_list" object you have in your environment.
  filter(...) %>%                                                   # 4 - Filter out all municipalities for which we do not know the share of votes for the political parties (they are coded as "*").
  mutate(...) %>%                                                   # 5 - Transform the column for each political party into numerical values.
  select(...) %>%                                                   # 6 - Select the columns related to some dimensions (e.g., 1, 2, 3, and 8) and the columns with the score of each political party.
  cor() %>%
  corrplot()
